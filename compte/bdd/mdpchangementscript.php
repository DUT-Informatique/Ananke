<?php
  ...
  if ($requeteVerification->rowCount() == 0) {
    ...
    $requeteInserer = $dbh->prepare("UPDATE Usager SET Verification = ?, MotDePasse = ?, clemdp = ? WHERE Identifiant = ?");
    $requeteInserer->execute(array("1",$pass_hache, $cle, htmlspecialchars($_POST["username"])));
    //--------------------------------------------------
    //mail
    ...
    $mail->Username = 'anankecontactclient@gmail.com';

    //Password to use for SMTP authentication

    $mail->Password = "69jUrY5WUY7T4GK";
    ...
    $mail->Body="<html><head><meta http-equiv='Content-Type' content='text/html; charset='UTF-8' /></head><body>Vous venez de changer de mot de passe.<body><html>";
    if (!$mail->send()) {
